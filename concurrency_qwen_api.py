...
# 设置日志记录(dify)
dify_logger = logging.getLogger("dify_logger")
dify_logger.setLevel(logging.INFO)
today = datetime.datetime.today().date().strftime('%Y-%m-%d')
os.makedirs('logs/qwen-14b-concurrency_dify', exist_ok=True)
dify_fh = logging.FileHandler(f'logs/qwen-14b-concurrency_dify/{today}.txt', encoding='utf-8')
dify_fh.setFormatter(logging.Formatter('%(asctime)s:%(levelname)s: %(message)s'))
dify_logger.addHandler(dify_fh)

# 设置日志记录（LEAP）
history = {}
leap_logger = logging.getLogger("leap_logger")
leap_logger.setLevel(logging.INFO)
today = datetime.datetime.today().date().strftime('%Y-%m-%d')
os.makedirs('logs/qwen-14b-concurrency_leap', exist_ok=True)
leap_fh = logging.FileHandler(f'logs/qwen-14b-concurrency_leap/{today}.txt', encoding='utf-8')
...
if __name__ == "__main__":
    uvicorn.run(app, host='0.0.0.0', port=8591)
